...
def test_pending_airdrop_id_constructor(mock_account_ids):
    """
    Test the constructor of the PendingAirdropId class for both fungible and non-fungible tokens.

    This test verifies that:
    - When a TokenId is provided, the PendingAirdropId is initialized correctly
      with the token_id set and nft_id as None.
    - When an NftId is provided, the PendingAirdropId is initialized correctly
      with the nft_id set and token_id as None.
    - The sender and receiver IDs are set properly in both cases.

    The constructor enforces that exactly one of token_id or nft_id must be provided;
    this test validates the expected behavior for valid construction paths.
    """
